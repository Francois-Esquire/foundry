...
interface GeneratorAgent extends Agent {
  ...
  generateFromTemplate(template: string, variables: Record<string, any>): Promise<string>;
...
interface LLMModel {
  ...
  stream(prompt: string, options?: GenerationOptions): AsyncIterableIterator<StreamChunk>;
...
class OpenAIModel implements LLMModel {
  ...
  async complete(prompt: string, options?: GenerationOptions): Promise<ModelResponse> {
    // Call OpenAI API
    return response;
  }
...
```typescript
async function generateTasksFromPRD(prdContent: string, foundry: Foundry): Promise<Task[]> {
...
```typescript
async function createNewTool(specification: string, foundry: Foundry): Promise<Tool> {
  // Generate the tool
